...
static Sdb *get_sdb(RBinFile *bf) {
	struct r_bin_coff_bigobj *bin = (struct r_bin_coff_bigobj *)R_UNWRAP3 (bf, bo, bin_obj);
	return bin? bin->kv: NULL;
}

static bool r_coff_is_stripped(struct r_bin_coff_bigobj *obj) {
	return !!(obj->hdr.f_flags & (COFF_FLAGS_TI_F_RELFLG | COFF_FLAGS_TI_F_LNNO | COFF_FLAGS_TI_F_LSYMS));
}

static bool load(RBinFile *bf, RBuffer *buf, ut64 loadaddr) {
	bf->bo->bin_obj = r_bin_coff_bigobj_new_buf (buf, bf->rbin->verbose);
	return bf->bo->bin_obj != NULL;
}

static void destroy(RBinFile *bf) {
	r_bin_coff_bigobj_free ((struct r_bin_coff_bigobj *)bf->bo->bin_obj);
}
...
static bool _fill_bin_symbol(RBin *rbin, struct r_bin_coff_bigobj *bin, int idx, RBinSymbol **sym) {
	RBinSymbol *ptr = *sym;
	struct coff_bigobj_symbol *s = NULL;
	...
	s = &bin->symbols[idx];
	char *coffname = r_coff_bigobj_symbol_name (bin, s);
	if (!coffname) {
		return false;
	}
	ptr->name = coffname;
...
static bool is_imported_symbol(struct coff_bigobj_symbol *s) {
	return s->n_scnum == COFF_SYM_SCNUM_UNDEF && s->n_sclass == COFF_SYM_CLASS_EXTERNAL;
}

static RBinImport *_fill_bin_import(struct r_bin_coff_bigobj *bin, int idx) {
	RBinImport *ptr = R_NEW0 (RBinImport);
	if (!ptr || idx < 0 || idx > bin->hdr.f_nsyms) {
		free (ptr);
		return NULL;
	}
	struct coff_bigobj_symbol *s = &bin->symbols[idx];
...
static RList *entries(RBinFile *bf) {
	struct r_bin_coff_bigobj *obj = (struct r_bin_coff_bigobj *)bf->bo->bin_obj;
	RList *ret;
	if (!(ret = r_list_newf (free))) {
		return NULL;
	}
	RBinAddr *ptr = r_coff_bigobj_get_entry (obj);
	if (ptr) {
...
static RList *sections(RBinFile *bf) {
	...
	if (obj && obj->scn_hdrs) {
		for (i = 0; i < obj->hdr.f_nscns; i++) {
			tmp = r_coff_bigobj_symbol_name (obj, &obj->scn_hdrs[i]);
			if (!tmp) {
				r_list_free (ret);
				return NULL;
			}
...
static RList *_relocs_list(RBin *rbin, struct r_bin_coff_bigobj *bin, bool patch, ut64 imp_map) {
...
static RList *relocs(RBinFile *bf) {
	struct r_bin_coff_bigobj *bin = (struct r_bin_coff_bigobj *)bf->bo->bin_obj;
...
static RList *patch_relocs(RBinFile *bf) {
	...
	struct r_bin_coff_bigobj *bin = (struct r_bin_coff_bigobj *)bo->bin_obj;
